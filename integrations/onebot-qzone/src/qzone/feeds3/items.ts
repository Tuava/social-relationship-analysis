...
export function parseFeeds3Items(
  ...
  while ((fdm = feedDataPat.exec(processedText)) !== null) {
    ...
    // Media belongs to the current feed, not to the window before its
    // feed_data marker. The latter can contain the previous feed's images
    // because feeds3 places the marker near the start of a card.
    const regionCurrentDecoded = htmlUnescape(searchAfterCurrentOnly);
    ...
    const EXCLUDED_IMAGE_PATTERNS = [
      ...
      /qzonestyle\.gtimg\.cn\/qzone_v6\/img\/feed\/(?:loading|gift-default)\./i,
      /\/ac\/b\.gif$/,
      /qlogo\.cn/,
      /qzapp\.qlogo\.cn/,
    ...
    const videoMatches = [...regionCurrentDecoded.matchAll(videoPat)];
    for (const vm of videoMatches) {
      const vid = vm[1] || vm[3];
      const videoUrl = vm[2];
      if (vid && !videos.some(v => v.videoId === vid)) {
        const coverMatch = regionCurrentDecoded.match(new RegExp(`data-vid="${vid}"[^>]*data-pic="([^"]+)"`, 'i'))
          || regionCurrentDecoded.match(new RegExp(`data-pic="([^"]+)"[^>]*data-vid="${vid}"`, 'i'));
    ...
    for (const vtm of regionCurrentDecoded.matchAll(videoTagPat)) {
      const src = vtm[1]!;
    ...
    if (qqVideoPat.test(regionCurrentDecoded) && videos.length === 0) {
      const qvideoPat = /class="[^"]*(?:qvideo|video-box|video-container)[^"]*"/gi;
      const qvideoMatch = regionCurrentDecoded.match(qvideoPat);
      if (qvideoMatch) {
        for (const container of qvideoMatch) {
          const containerMatch = regionCurrentDecoded.match(new RegExp(`${container.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[\\s\\S]*?<img[^>]+src="([^"]+)"`, 'i'));
    ...
    if (appid && appid !== '311') {
      // App metadata and media must use the same current-card boundary as
      // regular images; searchBefore may contain another feed card.
      const searchAll = searchAfterCurrentOnly;
