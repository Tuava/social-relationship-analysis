...
export class QzoneClient {
  ...
  isAuthFailure(payload: ApiResponse): boolean {
    ...
    // 目标空间权限受限（如 76BD-77 / 0103-87 / 0103-77 / 仅好友可见）属于目标空间锁，而非当前用户登录态失效
    if (/[0-9a-f]{4}-\d+|仅好友可见|仅对好友开放|无权访问|权限不足|空间被封/i.test(message)) return false;
  ...
  async getFeedsHtmlActAllPaginated(
    targetUin: string,
    opts: Parameters<QzoneClient['getFeedsHtmlActAll']>[1] = {},
    maxRounds = 100,
  ): Promise<{ result: ApiResponse }> {
    const count = Math.max(1, Math.min(50, Math.floor(opts?.count ?? 10)));
    const initial = Math.max(0, Math.floor(opts?.start ?? 0));
    const max = Math.max(1, Math.min(1000, Math.floor(maxRounds)));
    ...
    let stalled = false;
    for (; rounds < max;) {
      const page = await this.getFeedsHtmlActAll(targetUin, { ...opts, start, count });
      rounds++;
      if (page.code != null && Number(page.code) !== 0) return { result: page };
      const list = Array.isArray(page.msglist) ? page.msglist as Record<string, unknown>[] : [];
      let added = 0;
      for (const item of list) {
        const key = String(item.tid ?? item.cellid ?? '').trim() || JSON.stringify(item);
        if (seen.has(key)) continue;
        seen.add(key); merged.push(item); added++;
      }
      hasMore = Boolean(page.has_more);
      const next = typeof page.next_start === 'number' && Number.isFinite(page.next_start) ? page.next_start : start + count;
      if (!hasMore) break;
      if (list.length === 0 || added === 0 || next <= start) { stalled = true; break; }
      start = next;
    }
    const truncated = hasMore && rounds >= max;
    return { result: {
      code: 0, message: 'ok', msglist: merged, has_more: hasMore,
      next_start: hasMore ? start : undefined, partial: truncated || stalled,
      _page_info: { source: 'feeds_html_act_all', all_pages: true, rounds,
        per_page_count: count, start_initial: initial, unique_items: merged.length,
        truncated_by_max_rounds: truncated, stalled, max_rounds: max,
        request_uin: targetUin, feed_owner_uin: opts?.hostUin ?? this.qqNumber },
    } };
  }
  ...
  private async getEmotionListViaFeeds3(uin: string, num = 50, pos = 0, maxPages = 15, initialCursor = ''): Promise<ApiResponse> {
    try {
      ...
      // A repeated page means the upstream cursor stopped advancing. Retrying
      // ten times only burns requests and can make a sync look hung; retain
      // the already parsed items and expose the stall as partial data.
      ...
      const has_more = !emptySkipStreak && (buffer_has_more || (Boolean(next_cursor) && hasMoreFeedsOnLastPage));
  ...
  ): Promise<SdkResult<{ posts: Record<string, unknown>[]; nextCursor?: string; hasMore: boolean; partial: boolean; pageInfo?: PostPageInfo }>> {
    const targetUin = uin ?? this.qqNumber ?? '';
    try {
      ...
      return this.sdkSuccess(
        {
          ...
          partial: Boolean((res._page_info as PostPageInfo | undefined)?.partial || (res._page_info as PostPageInfo | undefined)?.pagination_stalled || (res._page_info as PostPageInfo | undefined)?.truncated_by_max_pages),
          pageInfo: res._page_info as PostPageInfo | undefined,
        },
  ...
  /**
   * 通过现代 users.qzone.qq.com/cgi-bin/likes/get_like_list_app 接口拉取点赞名单
   */
  async fetchLikesViaAppCgi(
    uin: string,
    tid: string,
    queryCount = 60,
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        unikey,
        begin_uin: '0',
        query_count: String(queryCount),
        if_first_page: '1',
        g_tk: String(this.getGtk()),
        format: 'json',
        rd: String(Math.random()),
      });
      const url = `https://h5.qzone.qq.com/proxy/domain/users.qzone.qq.com/cgi-bin/likes/get_like_list_app?${params.toString()}`;
      const resp = await this.get(url, {
        headers: this.pcHeaders(`https://user.qzone.qq.com/${this.qqNumber}`),
      });
      if (resp.status !== 200 || !resp.text) return [];

      const parsed = parseJsonp(resp.text) as Record<string, unknown>;
      if (!parsed || parsed['code'] !== 0) return [];

      const data = parsed['data'] as Record<string, unknown> | undefined;
      const likeInfo = (data?.['like_uin_info'] as Array<Record<string, unknown>>) ?? [];

      const results: Array<Record<string, unknown>> = [];
      for (const item of likeInfo) {
        const likerUin = String(item['fuin'] ?? '');
        const nick = String(item['nick'] ?? '').trim();
        if (likerUin) {
          results.push({
            uin: likerUin,
            nickname: nick || likerUin,
            tid,
            ownerUin: uin,
            portrait: item['portrait'] ?? '',
            gender: item['gender'] ?? '',
            addr: item['addr'] ?? '',
            _source: 'get_like_list_app',
          });
        }
      }
      return results;
    } catch (e) {
      log('WARNING', `fetchLikesViaAppCgi error for uin=${uin} tid=${tid}: ${e}`);
      return [];
    }
  }

  /**
   * 通过手机端 H5 like/list 页面拉取点赞名单（支持突破非好友权限限制）
   */
  async fetchLikesViaH5LikeList(
    uin: string,
    tid: string,
  ): Promise<Array<Record<string, unknown>>> {
    if (!this.qqNumber) return [];
    try {
      const unikey = `http://user.qzone.qq.com/${uin}/mood/${tid}`;
      const encodedUnikey = encodeURIComponent(unikey);
      const url = `https://h5.qzone.qq.com/like/list/${encodedUnikey}/${this.qqNumber}/likelist?_wv=2098179&hostuin=${uin}&appid=&feed_action_domain_type=1`;
      const resp = await this.get(url, {
        headers: {
          'User-Agent': 'Mozilla/5.0 (Linux; Android 14; Mobile) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/120.0.0.0 Mobile Safari/537.36 QQ/9.0.0',
          'Referer': 'https://h5.qzone.qq.com/',
        },
      });
      if (resp.status !== 200 || !resp.text) return [];

      const m = resp.text.match(/window\.syncData\s*=\s*({.*?});?\s*<\/script>/);
      if (!m) return [];

      const syncData = JSON.parse(m[1]) as Record<string, unknown>;
      const likelistData = syncData['likelistData'] as Record<string, unknown> | undefined;
      const rawList = (likelistData?.['list'] as Array<Record<string, unknown>>) ?? [];

      const results: Array<Record<string, unknown>> = [];
      for (const item of rawList) {
        const user = (item['user'] as Record<string, unknown>) ?? item;
        const likerUin = String(user['uin'] ?? user['fuin'] ?? '');
        const nick = String(user['nickname'] ?? user['nick'] ?? '').trim();
        if (likerUin && likerUin !== '0') {
          results.push({
            uin: likerUin,
            nickname: nick || likerUin,
            tid,
      ...
      log('WARNING', `fetchLikesViaH5LikeList error for uin=${uin} tid=${tid}: ${e}`);
      return [];
    }
  }

  /**
   * 获取点赞列表（优先 feeds3，兜底使用 get_like_list_app 与 手机端 H5 like/list）。
   */
  async getLikeListBestEffort(uin: string, tid: string): Promise<Array<Record<string, unknown>>> {
    ...
    try {
      ...
      if (likes.length > 0) {
        ...
        const normalized = this.normalizeFeeds3LikesForTool(likes);
        this.feeds3LikesCache.set(tid, normalized);
        return normalized;
      }
    ...
    // 2. 现代 CGI 兜底：调用 get_like_list_app
    try {
      log('INFO', `getLikeListBestEffort: 尝试调用 get_like_list_app 现代 CGI 兜底拉取 uin=${uin} tid=${tid}`);
      const appLikes = await this.fetchLikesViaAppCgi(uin, tid);
      if (appLikes.length > 0) {
        log('INFO', `getLikeListBestEffort: get_like_list_app 成功获取 ${appLikes.length} 个点赞`);
        this.feeds3LikesCache.set(tid, appLikes);
        this.lastFeeds3FailureByTid.delete(tid);
        return appLikes;
      }
    } catch (e) {
      log('WARNING', `getLikeListBestEffort: get_like_list_app 失败: ${e}`);
    }

    // 3. 手机端 H5 深度穿透：调用 fetchLikesViaH5LikeList（可突破非好友公开说说限制）
    try {
      log('INFO', `getLikeListBestEffort: 尝试调用手机端 H5 like/list 页面穿透拉取 uin=${uin} tid=${tid}`);
      const h5Likes = await this.fetchLikesViaH5LikeList(uin, tid);
      if (h5Likes.length > 0) {
        log('INFO', `getLikeListBestEffort: 手机端 H5 like/list 成功获取 ${h5Likes.length} 个点赞`);
        this.feeds3LikesCache.set(tid, h5Likes);
        this.lastFeeds3FailureByTid.delete(tid);
        return h5Likes;
      }
    } catch (e) {
      log('WARNING', `getLikeListBestEffort: fetchLikesViaH5LikeList 失败: ${e}`);
    }
  ...
  async getLikesBestEffort(uin: string, tid: string): Promise<SdkResult<{
    ...
    try {
      ...
      const source = likes.length > 0 ? (likes[0]?.['_source'] === 'get_like_list_app' || likes[0]?.['_source'] === 'h5_likelist' ? 'official_api' : 'feeds3_html') : 'derived';
      return this.sdkSuccess(
